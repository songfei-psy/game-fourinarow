...
class parallel_IBS:
    ...
    """
    Parallel IBS class for computing the negative log-likelihood of a simulator based model.
   ...
   num_samples_per_call: int, optional
    ...
    def __init__(
        ...
        self.max_samples = max_samples
        self.neg_logl_threshold = neg_logl_threshold
        # Add missing attributes for compute_logl compatibility
        self.vectorized = None
        self.acceleration = 1.5
        self.acceleration_threshold = 0.1
        self.vectorized_threshold = 0.1
        self.max_mem = 1e6
    ...
    def __call__(
        ...
        n_trials = self.response_matrix.shape[0]

        # weights vector should be a scalar or same length as number of trials
        weights = 1.0
        if trial_weights is not None:
            weights = trial_weights.reshape(-1)
        if not np.isscalar(weights) and len(weights) != n_trials:
            raise ValueError(
                "IBS:SizeMismatch",
                "Length of trial_weights must match the number of trials",
            )
        ...
        def compute_logl(self, params, num_reps, weights, return_positive, t0):
            ...
            elapsed_time = 0
            num_reps = int(num_reps)
            num_trials = n_trials  # Reference the outer variable
        ...
        if additional_output in [None, "none"]:
            ...
            if additional_output == "full":
                ...
                return EstimateResult(
                    ...
                    message=message,
                    elapsed_time=time.perf_counter() - t0,
                    num_samples_per_trial=num_samples_total / n_trials,
                    fun_count=fun_count,
                )
        else:
            raise ValueError(
